...
export const uploadDataUriToStorage = async (dataUri: string, path: string): Promise<{ downloadURL: string, storagePath: string }> => {
  ...
  console.log(`Attempting to upload data URI to Firebase Storage at path: ${path}. Bucket: gs://${storageRef.bucket}. Ensure CORS is configured.`);
  try {
    ...
    console.error(`Error uploading data URI to Firebase Storage (path: ${path}). Error:`, error);
    let friendlyMessage = `Image upload to Firebase Storage failed for path: ${path}.`;
    if (error.message && error.message.toLowerCase().includes('cors policy') && error.message.toLowerCase().includes('preflight')) {
      friendlyMessage += ' This is a CORS PREFLIGHT error. Your Firebase Storage bucket is not configured to accept requests from this web application\'s origin. Please apply the CORS configuration to your gs://' + storageRef.bucket + ' bucket using gsutil.';
    } else if (error.code === 'storage/unauthorized' || (error.message && error.message.toLowerCase().includes('cors'))) {
        friendlyMessage += ' This is likely a CORS configuration issue on your Firebase Storage bucket (gs://' + storageRef.bucket + '). Please verify your bucket\'s CORS settings allow this origin and the necessary methods/headers.';
    } else {
        friendlyMessage += ` Details: ${error.name} - ${error.message || 'Unknown error'}. Code: ${error.code || 'N/A'}`;
    }
...
export const addProductToFirestore = async (
  ...
  try {
    ...
    if (productData.imageUrl && productData.imageUrl.startsWith('data:image')) {
      ...
      const imageFileExtension = productData.imageUrl.substring(productData.imageUrl.indexOf('/') + 1, productData.imageUrl.indexOf(';base64')) || 'png';
      const imageFileName = `${productData.slug}-${Date.now()}.${imageFileExtension}`;
    ...
    console.error('Error in addProductToFirestore (could be during image upload or Firestore save): ', error.message, error);
